...
- Negative / cost: extras matrix breadth. (The former graspologic `numpy<2` vs markitdown conflict was eliminated by **ADR-028** — `leidenalg` declares no numpy pin, so the `[tool.uv].conflicts` block was removed.)
