...
| [`MEMSPINE_FLOWS.md`](./MEMSPINE_FLOWS.md) | **Flow graph** — canonical top-level engine graph (event log + rebuildable projectors) + code-traced write / conflict / retrieve / sleep mermaid sub-flows. |
...
| [`ECOSYSTEM_METHODOLOGY.md`](./ECOSYSTEM_METHODOLOGY.md) | **Pass #6 research survey** — 25 in-scope repos: §0 index, §1 cross-cutting patterns, §2 per-repo methodology + flows, §N implications. *(Full fresh re-run 2026-07-11.)* |
| [`ECOSYSTEM_MEMORY_TAXONOMY.md`](./ECOSYSTEM_MEMORY_TAXONOMY.md) | **Pass #6 memory-category taxonomy** — 235 type rows across all 25 engines → memspine 9-type crosswalk. |
| [`ECOSYSTEM_PROMPTS.md`](./ECOSYSTEM_PROMPTS.md) | **Pass #6 full prompt catalog** — 689 verbatim prompts + wiring metadata (see also staging). |
...
| [`exports/ECOSYSTEM_REPO_SYNC.csv`](./exports/ECOSYSTEM_REPO_SYNC.csv) | **One-shot git sync ledger** — SHA/branch/pull status (Pass #6 freeze, 2026-07-11; 25 in-scope + 6 excludes). |
...
3b. **Pass #6:** `ECOSYSTEM_METHODOLOGY.md` (§1 cross-cutting + §N implications) + `ECOSYSTEM_MEMORY_TAXONOMY.md` + `ECOSYSTEM_PROMPTS.md` (staging for full prompt bodies)  
