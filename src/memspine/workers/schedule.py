...
consolidate → reorganize → check_watches → decay_sweep → compress →
event_log_prune, with the E7 sleep-time-compute hook slot reserved after
compress (no-op default, RG tier). The D-42 reorganizer runs right after
consolidate so fresh summaries join the graph the same cycle communities are
detected; it self-skips without a graph store or the ``[community]`` extra.
