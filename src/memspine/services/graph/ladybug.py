"""LadybugDB graph store — the intended embedded default (D-26), ``[graph]``.

LadybugDB published v0.18.0 on PyPI on 2026-07-01 (``pip install ladybug``,
top-level import ``ladybug``) — the fork is real and this adapter is real.
Verified against the installed package and the official docs
(docs.ladybugdb.com): the fork kept Kuzu's embedded-Cypher Python API and DDL
dialect unchanged (``Database``/``Connection``, synchronous client,
``CREATE NODE/REL TABLE IF NOT EXISTS`` with ``FROM ... TO ...`` rel tables),
so this adapter mirrors :class:`~memspine.services.graph.kuzu.KuzuGraphStore`
line-for-line via an injected :class:`~memspine.clients.ladybug.LadybugClient`
(D-22/D-24).

Despite that, ``graph.provider`` still defaults to ``sqlite_adjacency``
(zero-dep, D-03 slim core) — flipping the default to ``ladybug`` now that it
is installable is a deliberate follow-up decision left for a future ADR, not
made unilaterally here (see ADR-015 §5 / D-49's amendment).

One node table (``MemoryNode``) and one rel table (``MemoryLink``) are
created lazily on first use; labels/properties ride as canonical orjson
strings so the port's free-form payloads survive ladybug's fixed columnar
schema. Ladybug's Python API is synchronous — every call is pushed to a
worker thread, and a single lock serializes them (one embedded connection is
not a pool).
"""
...
class LadybugGraphStore:
    def __init__(self, client: LadybugClient) -> None:
        self._client = client
        self._ready = False
    ...
    async def close(self) -> None:
        """No-op: the injected LadybugClient owns the database handle (D-24)."""
